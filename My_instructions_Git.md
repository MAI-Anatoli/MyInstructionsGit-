...
## Но чтобы начать полноценно работать с *__GitHub.com__* надо зарегистрироватся. После регистрации мы можем создать на сайте свой первый репозиторий. Для этого на своей странице находим кнопочку с названием *__New__* нажимаем на нее и переходим на страницу где будем создавать свой репозитории. На странице мы увидим пустую ячейку для заполнения с названием *__Repository name__* мы туда записываем имя проекта. Смотрим чтобы было выбрано пункт *__Public__* иначе не хто не увидит и не сможет в дальнейшем вам помогать в проекте , или повторить его, спускаемся ниже и нажимаем кнопку *__Creating repository...__*.            
## Чтобы начать работать с репозиторием который мы создали нужно выполнить инструкцию которую нам выдает сам сайт *__GitHub__*.
...
```
## Чтобы отправить свою версию (измененую, обновленую) репозиторию необходимо использовать следующую команду:
```sh
git push -u origin main
```
## После выполнения этой команды вас попросят авторизоватся (связать git с __*GitHub*__) это в том случае если вы будете использовать ссылку *__https://__* но если вы зделаете связь с помошю ключа *__ssh__* то у вас просто ваш репозиторий отправится без проблем в __*GitHub*__.

## Чтобы вытянуть репозитории из __*GitHub*__ надо использовать команду:
```sh
git pull 
```
## С помощью команды _**git push**_ можно отправлять не только главные ветки __*main*__ но и другие которых мы зделали но нужно переключится не них с помощью команды *__git checkout name_branch__*

## После чего мы отправили все ветки на удаленный репозиторий мы можел сделать слияние и удалить ненужную ветку (черновик). Удаление веток можно сделать как в браузере так и с помощью команды при отправке ветки мы можем дополнить команду следующим образом:
```sh
...
git pull --rebase
```
## Ключ __*--rebase*__ он скачает изменения и сразу же сделает слияния с нашим локальным репозиториеи. И после скачивание и слияние и решения конфликтов и сделали индексацию и коммит. Мы можем отправить наш измененый репозиторий.